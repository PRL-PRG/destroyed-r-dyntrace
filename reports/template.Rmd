...
name <- "%%NAME%%"
aggregate_data <- "%%LOGS%%"
if (!dir.exists(aggregate_data))
  dir.create(aggregate_data)
store <- function(table) function(...) suppressWarnings(log_line_to_csv(file.path(aggregate_data, paste(table, "csv", sep=".")), name=name, ...))
...
<div class="col2">
``` {r basic-info-logging, include=FALSE}
...
store("fuzzy_forces")(fuzzy.force.histogram)
```
...
store("promise_evaluations")(promise.evaluation.histogram_with_labels)
```
<!--/div-->
</center>
...
store("promise_types")(promise.types)
```
</div>

<!--div class="together"-->
``` {r promise-full-types-pie, echo=FALSE, fig.width=5, fig.height=4.5, comment=""}
promise.full.types <- get_full_promise_types(cutoff=NA)
...
store("promise_full_types")(promise.full.types)
```
<!--/div-->
</div>
...
store("return_types")(return.types)
```
</div>
...
store("return_types_for_BCODE")(return.types)
```
-->`

``` {r promise-types-to-return-types-pie, echo=FALSE, comment="", fig.width=5, fig.height=4.5}
types.to.return.types <- get_promise_types_to_return_types(cutoff=NA)

slices <- (types.to.return.types %>% arrange(number))$number
labels <- (types.to.return.types %>% arrange(number) %>% mutate(types = ifelse(percent>1, types, "")))$types
par(mar=c(0,2,2,2))

pie(slices, labels, main="Promise code → return types")

if(print_data) print(types.to.return.types %>% as.data.frame %>% transform(number=pp(number), percent=pp(percent)), row.names=FALSE)

store("promise_code_to_return_types")(types.to.return.types)
```
...
store("forces_by_type")(force.histogram.by.type)
```
</center>
...
store("promise_lifestyles")(promise.lifestyles)
```
</center>
-->
...
store("effective_distances")(histogram_with_labels)
...
<center>
<!--div class="together"-->
``` {r actual-distances, echo=FALSE, fig.width=14, fig.height=5, comment=""}
cutoff <- 20
...
store("actual_distances")(histogram_with_labels)
```
<!--/div-->
<!--/div-->
</center>
...
store("call_types")(calls.types)
```
</div>

```{r functions-by-type, echo=FALSE, fig.width=5, fig.height=4.5, comment=""}
function.types <- get_functions_by_type()

slices <- (function.types %>% arrange(number))$number
labels <- (function.types %>% arrange(number) %>% mutate(type = ifelse(percent>1, type, "")))$type
par(mar=c(0,2,2,2))

pie(slices, labels, main="Function types")
if(print_data) print(function.types %>% as.data.frame %>% transform(number=pp(number), percent=pp(percent)), row.names=FALSE)

store("function_types")(function.types)
```
</div>
...
<div class="col2">
``` {r compiled-calls-closures, echo=FALSE, comment="", fig.width=5, fig.height=4.5, warning=FALSE}
...
store("compiled_calls")(closure.call.compilation.histogram)
```
<div class="together">

</div>
<!--div class="together"-->
``` {r compiled-functions-closures, echo=FALSE, comment="", fig.width=5, fig.height=4.5, warning=FALSE}
...
store("compiled_functions")(closure.function.compilation.histogram)
```
<!--/div-->
</div>
...
store("promises_forced_by_another_promise")(histogram)
```
...
store("promises_forcing_other_promises")(histogram)
```
...
store("call_strictness")(strict.calls.histogram)
```
...
store("call_strictness_by_type")(strict.calls.by.type.histogram)
```
...
store("call_strictness_ratio")(strict.calls.ratio.histogram)
````
</center>
...
store("function_strictness_by_type")(strict.functions.by.type.histogram)
```
</div>
...
store("function_strictness_rate")(strict.functions.rate.histogram)
````
</center>
...
store("evaluation_order")(promise.force.order.histogram_with_labels)
```
...
specific.calls <- get_function_calls("^<anonymous>$", "eval", "force", "delayAssign", "return", "<-", "assign", "print") %>% select(function_name, number, percent)

print(specific.calls %>% data.frame %>% transform(number=pp(number), percent=pp(percent)), row.names=FALSE)

store("specific_calls")(specific.calls)
```
